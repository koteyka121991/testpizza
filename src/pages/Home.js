...
const Home = () => {
  // useDispatch хук возвращает ссылку на dispatch функцию из хранилища Redux.
  // дай нам функцию которая меняет стэйт
  const dispatch = useDispatch();
  // состояния активного индекса категории через реакт при помощи useState
  // const [categoryId, setCategoryId] = React.useState(0);
  // useSelector при помощи этого хука мы вытаскиваем весь стэйт
  // (state=>state.filter.categoryId) тут указываем что мы хотим вытащить из стэйта
  ...
  const onClickCategory = (i) => {
    // setCategoryId  метод из filterSlice который меняет стэйт 
    // тут мы хотим озвучить что хотим поменять катгегрию на новую
    ...
    <>
      <div className='container'>
        <div className="content__top">
          ...
          <Sort 
          // sortValue={sortType} onClickType={(i) => setSortType(i)} 
          />
        </div>
